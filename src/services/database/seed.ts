...
export const seedDatabase = async () => {
  ...
  const existing = await db.query('SELECT COUNT(*) as count FROM cycles');
  if ((existing.values?.[0]?.count ?? 0) > 0) return;

  console.log('[Seed] Seeding cycles and modules...');

  // ─── CICLOS ──────────────────────────────────────────────────
  await db.run(`INSERT INTO cycles VALUES
  ...
  // ─── MÓDULOS DO CICLO I ───────────────────────────────────────
  const cycle1Modules = [
    { id: 'C1-M01', order: 1, title: 'Vogais Base: Α, Ε, Ι', anchor: 'ἠγάπησεν', ref: 'João 3:16', method: 'Flashcard + Canvas 2D + Áudio', xp: 60, exercises: 10 },
    ...
    { id: 'C1-M06', order: 6, title: 'Oclusivas Β, Δ, Γ', anchor: 'κληρονομήσουσιν τὴν γῆν', ref: 'Mateus 5:5', method: 'Canvas 2D + Múltipla Escolha + TPR', xp: 65, exercises: 10 },
