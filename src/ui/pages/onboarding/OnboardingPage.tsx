            ...
            {SLIDES.map((_, i) => (
              <div
                ...
                className={`rounded-full transition-all ${
                  i === step ? 'w-6 h-2 bg-primary dark:bg-secondary' : 'w-2 h-2 bg-border dark:bg-border/50'
                }`}
              />
            ))}
          </div>
          <Button label="Próximo" onClick={handleNext} fullWidth radius="full" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
            ...
            <button
              ...
              className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all ${
                selectedGoal === goal.type
                  ? 'border-primary dark:border-secondary bg-primary/5'
      ...
      <div className="p-6 pb-safe">
        <Button label="Começar a Aprender" onClick={handleNext} fullWidth variant="primary" radius="full" />
      </div>
    </div>
  );
};
